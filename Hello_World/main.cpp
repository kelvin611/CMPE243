...
CMD_HANDLER_FUNC(displaySensor)
{
    /* cmdParams is a str passed to you after user's command.
     * If command was "newcmd test 123" then cmdParams will be "test 123".
     *
     ...
     * See "handlers.cpp" for more examples
    */
    float temperature_F = TS.getFarenheit();
    float temperature_C = TS.getCelsius();
    int light_value = LS.getRawValue();
    int light_percentage = LS.getPercentValue();
    int tilt_x = AS.getX();
    int tilt_y = AS.getY();
    int tilt_z = AS.getZ();


    if (cmdParams == "temperature")
    {
        output.printf("Current temperature is %.1fF (%.2fC).\n", temperature_F, temperature_C);
    }

    else if (cmdParams == "light")
    {
        output.printf("Current light value is %d (%d\%%).\n", light_value, light_percentage);
    }

    else if (cmdParams == "acceleration")
    {
        output.printf("X-axis: %d \nY-axis: %d \nZ-axis: %d\n", tilt_x, tilt_y, tilt_z);
    }
